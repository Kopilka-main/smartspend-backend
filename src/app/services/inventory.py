...
def _item_to_response(item: InventoryItem) -> InventoryItemResponse:
    return InventoryItemResponse(
        id=item.id, user_id=str(item.user_id), group_id=item.group_id,
        type=item.type, name=item.name, price=item.price,
        set_id=item.set_id, is_extra=item.is_extra, paused=item.paused,
        notes=item.notes, qty=item.qty, unit=item.unit,
        daily_use=item.daily_use, last_bought=item.last_bought,
        wear_life_weeks=item.wear_life_weeks, purchase_date=item.purchase_date,
        purchases=[
            InventoryPurchaseResponse(
                id=p.id, position=p.position,
                bought=p.bought, purchase_date=p.purchase_date,
            )
        ...
        photos=[
            InventoryPhotoResponse(
                id=ph.id, url=ph.url,
                file_name=ph.file_name, created_at=ph.created_at,
            )
...
class InventoryService:
    ...
    async def create_item(self, user: User, data: InventoryItemCreate) -> InventoryItemResponse:
        ...
        item = InventoryItem(
            id=item_id, user_id=user.id, group_id=data.group_id,
            type=data.type, name=data.name, price=data.price,
            set_id=data.set_id, is_extra=False, paused=False,
            notes=data.notes, qty=data.qty, unit=data.unit,
            daily_use=data.daily_use, last_bought=data.last_bought,
            wear_life_weeks=data.wear_life_weeks, purchase_date=data.purchase_date,
        )
    ...
    async def update_item(
        self, item_id: str, user_id: uuid.UUID, data: InventoryItemUpdate
    ) -> InventoryItemResponse:
        item = await self._repo.get_by_id(item_id)
        if item is None or item.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
    ...
    async def activate_item(
        self, item_id: str, user_id: uuid.UUID, data: ActivateItemRequest
    ) -> InventoryItemResponse:
        item = await self._repo.get_by_id(item_id)
        if item is None or item.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
    ...
    async def restock_item(
        self, item_id: str, user_id: uuid.UUID, data: RestockRequest
    ) -> InventoryItemResponse:
        item = await self._repo.get_by_id(item_id)
        if item is None or item.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
    ...
    async def replace_item(
        self, item_id: str, user_id: uuid.UUID, data: ReplaceRequest
    ) -> InventoryItemResponse:
        item = await self._repo.get_by_id(item_id)
        if item is None or item.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
    ...
    async def reassign_set(
        self, item_id: str, user_id: uuid.UUID, new_set_id: str
    ) -> InventoryItemResponse:
        item = await self._repo.get_by_id(item_id)
        if item is None or item.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
    ...
    async def add_photo(
        self, item_id: str, user_id: uuid.UUID, url: str, file_name: str
    ) -> InventoryPhotoResponse:
        item = await self._repo.get_by_id(item_id)
        if item is None or item.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
        photo = InventoryPhoto(item_id=item_id, url=url, file_name=file_name)
        photo = await self._repo.add_photo(photo)
        await self._session.commit()
        return InventoryPhotoResponse(
            id=photo.id, url=photo.url,
            file_name=photo.file_name, created_at=photo.created_at,
        )
